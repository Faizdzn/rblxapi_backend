...
class RBX {
  ...
  // Game Section
  async getGameDetail(univ_id) {
    const id = parseInt(univ_id) || 1;
    const gameApi = `https://games.roblox.com/v1/games?universeIds=${id}`;
    ...
    try {
      ...
      if(data['data'].length < 1) {
        ...
        let res = data.data[0];
        res['gameIcon'] = await this.getGameIcon(res['id']) || null;
        return res;
      }
    } catch(err) {
      throw err;
  ...
  async getGameIcon(univ_id) {
    const api = `https://thumbnails.roblox.com/v1/batch`;
    const id = parseInt(univ_id);
    
    const payload = [
      {
        "requestId": `${id}::GameIcon:150x150:webp:regular`,
        "type": "GameIcon",
        "targetId": id,
        "format": "webp",
        "size": "150x150"
  ...
  async getUIDbyUsername(username) {
    const apiSearch = `https://users.roblox.com/v1/usernames/users`;
    const payload = {
      usernames: [username],
    ...
    try {
      const { data } = await axios.post(apiSearch, payload);
