...
class RBX {
  ...
  async getGroupDetail(gid) {
    ...
    try {
      ...
      data['totalActivePlay'] = playing;
      data['totalVisit'] = visit;
      data['totalFav'] = fav;
      return data;
    } catch(err) {
      throw err;
  ...
  async getGroupGames(gid) {
    ...
    try {
      ...
      if(data['data'].length < 1) {
        throw new RBXException("E_GANENOTFOUND");
      } else {
  ...
  async getGameDetail(game_id) {
    ...
    try {
      let { data } = await axios.get(gameApi);
      data = data.data[0];
      data['gameIcon'] = await this.getGameIcon(id);
